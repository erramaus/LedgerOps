...
export const quickBooksService = {
  getStatus: async (): Promise<QuickBooksStatus> => {
    const response = await fetch(apiUrl('/api/quickbooks/status'))
    if (!response.ok) throw new Error('Unable to check QuickBooks status.')
    return response.json() as Promise<QuickBooksStatus>
  },
  connect: () => {
    window.location.href = apiUrl('/api/quickbooks/connect')
  },
  getCompany: async (): Promise<QuickBooksCompany> => {
    const response = await fetch(apiUrl('/api/quickbooks/company'))
  ...
  getTransactions: async (): Promise<LedgerTransaction[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/transactions'))
  ...
  getAccounts: async (): Promise<LedgerAccount[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/accounts'))
  ...
  getCustomers: async (): Promise<LedgerContact[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/customers'))
  ...
  getVendors: async (): Promise<LedgerContact[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/vendors'))
  ...
  getInvoices: async (): Promise<LedgerInvoice[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/invoices'))
  ...
  getBills: async (): Promise<LedgerBill[]> => {
    const response = await fetch(apiUrl('/api/quickbooks/bills'))
  ...
  getReport: async (report: 'profit-loss' | 'balance-sheet' | 'cash-flow', period: string): Promise<LedgerReport> => {
    const response = await fetch(apiUrl(`/api/quickbooks/reports/${report}?period=${encodeURIComponent(period)}`))
  ...
  updateTransactionAccount: async (transactionType: string, transactionId: string, accountName: string) => {
    const response = await fetch(apiUrl(`/api/quickbooks/transactions/${encodeURIComponent(transactionType)}/${encodeURIComponent(transactionId)}/account`), {
      method: 'POST',
